...
public class ChessMatch {
	...
	public boolean[][] possibleMovies(ChessPosition source) {
	...
	public ChessPiece replacePromotedPiece(String type) {
		if(promoted == null) {
			throw new IllegalStateException("Não possui peças a ser promovida");
		}
		if(!type.equalsIgnoreCase("B") && !type.equalsIgnoreCase("H") && !type.equalsIgnoreCase("Q") && !type.equalsIgnoreCase("T")) {
			throw new InvalidParameterException("Tipo de promoção invalida, selecione uma das seguintes: B, H, Q, T");
		}
	...
	private ChessPiece newPiece(String type, Color color) {
		...
		if(type.equalsIgnoreCase("H")) return new Knight(board, color);
	...
	private Piece makemove(Position src, Position tgrt) {
		...
		// lil castling logic kingside rook
		if (p instanceof King && tgrt.getColumn() == src.getColumn() + 2) {
		...
		// big castling logic kingside rook
		if (p instanceof King && tgrt.getColumn() == src.getColumn() - 2) {
		...
		if (p instanceof Pawn) {
			if (src.getColumn() != tgrt.getColumn() && capturedPiece == null) {
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
					pawnPosition = new Position(tgrt.getRow() + 1, tgrt.getColumn());
				} else {
					pawnPosition = new Position(tgrt.getRow() - 1, tgrt.getColumn());
				}
	...
	private void validateSourcePosition(Position ps) {
		...
		if (currentPlayer != ((ChessPiece) board.piece(ps)).getColor()) {
			throw new ChessException("A peça escolhida não pertence ao seu arsenal");
		}

		if (!board.piece(ps).isThereAnyPossibleMOve()) {
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("O tabuleiro não possui um rei da cor: " + color);
	}
